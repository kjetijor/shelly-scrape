...
def create_metrics(commonlabels: Mapping[str,str], metrics: Mapping[str, Tuple[PlugConfig, PlugMetrics]]) -> CollectorRegistry:
    labelnames, labelvalues = zip(*commonlabels.items()) if len(commonlabels.keys()) > 0 else ([], [])
    reg = CollectorRegistry()
    labelnames = list(labelnames) + ["scrapehost", "location", "device", "name"]
    apower = Gauge("shellyplug_apparent_power", "apparent power", labelnames, registry=reg)
    ...
    totenergy = Gauge("shellyplug_apparent_energy_usage_total", "total energy usage in WH", labelnames, registry=reg)
    aenergy = Gauge("shellyplug_apparent_energy_usage", "apparent energy usage in mWH", labelnames + ["bucket"], registry=reg)
    temperature = Gauge("shellyplug_temperature", "temperature", labelnames + ["unit"], registry=reg)
    for (scrapehost, (config, plugmetrics)) in metrics.items():
        plugvalues = list(labelvalues) + [scrapehost, config.location, config.device, config.name]
...
def write_metrics(metricsdir: Path, reg: CollectorRegistry, filename="shellyscrape.prom"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--outdir", "-o", type=Path, default=OUTDIR, help="output directory")
    parser.add_argument("--config", "-c", type=Path, default="/etc/shelly-scrape/shelly-scrape.yaml", help="plug labels file")
    parser.add_argument("--filename", "-f", type=str, default="shellyscrape.prom", help="output filename")
    parser.add_argument("--add-plug", "-P", type=str, action="append", default=[], help="plug hostnames")
    parser.add_argument("--interval", "-i", type=int, default=3, help="interval in seconds")
    parser.add_argument("--no-cleanup", action="store_true", default=False, help="don't cleanup on exit")
    parser.add_argument("--once", action="store_true", default=False, help="run once and exit")
    ...
    print(f"Plugs with config {','.join(scrapecfg.plugs.keys())}, ad-hoc plugs {','.join(args.add_plug)}", file=sys.stderr)
    targets: Mapping[str, PlugConfig] = {}
    for (plug, cfg) in scrapecfg.plugs.items():
        targets[plug] = cfg
    for plug in args.add_plug:
        if plug in targets.keys():
    ...
    print(f"Starting with interval {args.interval}s, outputting metrics to {args.outdir}/{args.filename}", file=sys.stderr)
    while not donehandler.done():
        ...
        for (plug, cfg) in targets.items():
            try:
                ...
                print(f"Failed to scprape {plug}, failed to connect with {conerr}", file=sys.stderr)
